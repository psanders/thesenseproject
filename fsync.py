...
FTP_SEC = [CMD_AT_FTPSERV, CMD_AT_FTPPORT, CMD_AT_FTPPUN, CMD_AT_FTPPW, CMD_AT_FTPPUTPATH]
...
def setup_ftp():
    for i in FTP_SEC:
        ...
        print(response);
def upload_ftp(file):
    CMD_AT_FTPPUTNAME[0] = CMD_AT_FTPPUTNAME + "\"" + file "\""
    response = send_cmd(CMD_AT_FTPPUTNAME)
    print response
